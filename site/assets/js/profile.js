...
var Profile = function () {
				...
				submitHandler: function (form) {
					$(form).ajaxSubmit({
						...
						success: function(jsonData, statusText, xhr, jqForm){
							...
							$(jqForm[0]).find('button[type="submit"]').html('Confirmar').removeAttr('disabled');
							...
							if (jsonData.status == true){
								...
								WebDialogs.doAlert({
									...
									onConfirm: function(){
										window.location.href = __SITEURL+'profile';
									}
								});
	...
	var handleImageThumb = function(){
		...
		$('#frmSaveImagenPerfil').each(function(){
			var aForm = $(this);
	    ...
	    $("#upload").change(function(){
			...
			var extension = control.val().substr( (control.val().lastIndexOf('.') +1) );
			if ($.inArray(extension.toLowerCase(), fileExtension) == -1) { 
				WebDialogs.doAlertError({
					message: 'Los formatos de imagen deben ser: '+fileExtension.join(', '),
					title: 'Error'
				});	
				//alert("Only formats are allowed: "+fileExtension.join(', '));
			}else{
				
				$('#frmSaveImagenPerfil').submit();
	        	readURL(this);
