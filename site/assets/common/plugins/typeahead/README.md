...
* `grunt build` â€“ Builds *typeahead.js* from source.
* `grunt lint` â€“ Runs source and test files through JSHint.
* `grunt watch` â€“ Rebuilds *typeahead.js* whenever a source file is modified.
* `grunt server` â€“ Serves files from the root of typeahead.js on localhost:8888. 
  Useful for using *test/playground.html* for debugging/testing.
* `grunt dev` â€“ Runs `grunt watch` and `grunt server` in parallel.
