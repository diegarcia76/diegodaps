...
Installation of Uniform is quite simple. First, make sure you have jQuery installed. Then youâ€™ll want to link to the jquery.uniform.js file and uniform.default.css in the head area of your page.  Here's what your `<head>` tag contents should probably contain:
...
If useID is set to true, this string is prefixed to element IDâ€™s and attached to the container div of each Uniformed element. If you have a checkbox with the ID of "remember-me" the container div would have the ID "uniform-remember-me".
...
If you need to change values on the form dynamically you must tell Uniform to update that elementâ€™s style. Fortunately, itâ€™s very simple. Just call this function, and Uniform will do the rest.

    $.uniform.update("#myUpdatedCheckbox");

If you don't mind updating all Uniformed elements or just donâ€™t specifically know which element to update, you can just leave out the parameter (see below) and Uniform will update all Uniformed elements on the page:
...
* Remember to change the CSS classes in the theme if you change the parameters for elementsâ€™ classes. This can be tedious work, but if you donâ€™t do it, itâ€™s not going to look correct. Find and Replace is your friend.
...
* Uniform is disabled in IE6. Itâ€™s not possible to fix due to the way IE6 handles form elements. If you care about IE6 users, give it a quick look to make sure your "naked" form elements look alright in there.
