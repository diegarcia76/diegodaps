...
var Deudores = function() {
	...
	var handleCambioTurno = function (){
		...
		$('.btn-confirm-efectivo').click(function(){
			...
			$.ajax({
				url: __SITEURL+'admin/deudores/pagar/'+pago_id,
				data: {
					tipo:'efectivo',
		...
		//alert(pago_id);
		//var precio = parseFloat($('#confirmarCobro_'+pago_id+' t').val();
		//alert(precio);
		//var pago_id = parseInt($(this).data('id-pago'));
		...
		//$('.cuotas_'+pago_id).html('<i class="fa fa-check"></i>Cobrar $'+subt+'.-');
	...
	var handleEliminarTodos = function(){
		$('#eliminar_todos').on('click', function(e){
			...
			$.ajax({
				url: __SITEURL+'admin/deudores/eliminar_todos/',
				data: {
					lote:lote
				},
				type: 'POST',
				...
				WebDialogs.doConfirmSecond({
					...
					onConfirm: function(){
						...
						WebDialogs.doConfirm({
							...
							onConfirm: function(){
								$.ajax({
									url: __SITEURL+'admin/deudores/eliminarItem/'+detalle_id,
									type: 'POST',
									dataType: 'json',
									success: function (jsonData){					
										//alert(jsonData.message);
	...
	var handleEditarItem = function(){
		...
		$('.btn-modificar-item').click(function(){
			detalle_id = $(this).data('id-detalle');
			$.ajax({
				url: __SITEURL+'admin/deudores/getDetallePago/'+detalle_id,
				type: 'POST',
				dataType: 'json',
				success: function (jsonData){					
					...
					if(jsonData.detallePago.producto_id>0){
						...
						$('#modal-agregar-producto')
							.find('form').attr('action',__SITEURL+'admin/deudores/editProducto');
						...
						$('#modal-agregar-servicio')
							.find('form').attr('action',__SITEURL+'admin/deudores/editServicio');
						...
						$('#modal-agregar-item')
							.find('form').attr('action',__SITEURL+'admin/deudores/editItem');
	...
	var handleEliminarTodosPagos = function(){
		$('#eliminar_todos_pagos').on('click', function(e){
			...
			$.ajax({
				url: __SITEURL+'admin/deudores/eliminar_todos_pagos/',
				data: {
					lote:lote
				},
				type: 'POST',
