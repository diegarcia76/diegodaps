...
var Usuarios = function() {
	...
	var handleTable = function(){
		...
		var table = $("#tblUsuarios").DataTable({
			...
			"order": [[ 0, "desc" ]],
			ajax: {
				url: __SITEURL+'admin/usuarios/datasource',
				type: 'POST'
			},
	...
	var handleDelete = function(){
		$('#tblUsuarios').on('click','a.btn-eliminar', function(e){
			e.preventDefault();
			var usuarioNombre = $(this).attr('title');
			var href = $(this).attr('href');
			WebDialogs.doConfirm({
				message: 'Esta a punto de eliminar el Usuario <strong>'+usuarioNombre+'</strong>. Si esta seguro que desea eliminar el Usuario haga click en aceptar, de lo contrario cancele la opción.',
			...
			WebDialogs.doConfirm({
				...
				onConfirm: function(){
					...
					$.ajax({
						...
						success: function (jsonData){
							if (jsonData.status == true){
								window.location.href = __SITEURL + 'admin/usuarios';
							} else {
								alert(jsonData.message);
							}
						}
